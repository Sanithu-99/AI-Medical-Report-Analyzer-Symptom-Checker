...
class Predictor:
    ...
    def predict_from_symptoms(self, symptoms: str) -> List[str]:
        structured = self.predict_proba_from_symptoms(symptoms)
        if structured:
            return [f"{label}: {probability:.2%}" for label, probability in structured[:3]]
        return self.artifacts.fallback(symptoms)

    def predict_proba_from_symptoms(self, symptoms: str) -> List[Tuple[str, float]]:
        if not self._is_vectorizer_ready():
            return []
        ...
        if hasattr(self.artifacts.model, "predict_proba") and getattr(self.artifacts.model, "classes_", None) is not None:
            ...
            classes = getattr(self.artifacts.model, "classes_", [])
            sorted_indices = np.argsort(probabilities)[::-1]
            return [(classes[idx], float(probabilities[idx])) for idx in sorted_indices]
        return []
